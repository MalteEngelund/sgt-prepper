...
export const ProductListView = (products) => {
    ...
    products.forEach(product => {
        ...
        const div = Div('border flex m-10 bg-sky-50 shadow-lg hover:shadow-2xl rounded-xl p-4 max-sm:m-2 max-sm:mt-10 max-sm:flex-col max-sm:items-center')
        ...
        const h2 = Heading(name, 2, 'max-sm:text-center')
        const p = Paragraph()
        p.innerHTML = teaser
        info.append(h2, p)
        div.append(info)

        const cost = Div('text-right p-4 ml-auto max-sm:ml-0')
